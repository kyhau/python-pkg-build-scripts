...
GDAL_ROOT = os.path.abspath(os.path.join('..', '..', '..'))
BINARIES = []
import platform
...
if platform.system() == 'Windows':
    BINARIES = [
        ['out', 'bin', 'gdal200.dll'],
        ['pkg', 'bin32', 'FileGDBAPI.dll'],
        ['pkg', 'bin32', 'geos_c.dll'],
        ['pkg', 'bin32', 'proj.dll'],
    ]

BINARIES = [os.path.abspath(os.path.join(GDAL_ROOT, *x)) for x in BINARIES]
for src in BINARIES:
    import shutil

    dst = os.path.join(os.path.dirname(__file__), 'osgeo', os.path.basename(src))
